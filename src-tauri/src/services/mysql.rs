use super::{bin_path, hidden_command, kill_tree, posix, service_command, Service, ServiceStatus};
use std::fs;
...
impl MysqlService {
    ...
    fn ensure_initialized(&self) -> Result<(), String> {
        ...
        // `--no-defaults` has to come first, and is not optional: without it
        // mysqld reads /etc/my.cnf and /etc/my.cnf.d/. On any machine with a
        // distro MariaDB that pulls in MariaDB-only options
        // (`provider_bzip2`, `plugin_load_add`, …) and initialization aborts
        // with "unknown variable" before the data dir is usable. Everything
        // this needs is passed explicitly.
...
impl Service for MysqlService {
    fn start(&mut self) -> Result<(), String> {
        ...
        let mut cmd = service_command(&mysqld);
        cmd.arg(format!("--defaults-file={}", conf.display()));
        // `--console` routes the error log to stderr and is a Windows-only
        // option, Unix mysqld exits with "unknown variable" on sight of it.
        // stderr is captured either way, so dropping it on Unix loses nothing.
        #[cfg(windows)]
        cmd.arg("--console");
        let child = cmd
            .stdout(log_file)
...
fn build_minimal_conf(mysql_dir: &Path, data_dir: &Path, port: u16) -> String {
    ...
    // On Unix, clients reach a local server through a socket file, and mysqld
    // defaults it to a shared location (/tmp/mysql.sock or /var/run/mysqld/).
    // The first collides with a distro MySQL/MariaDB the user already runs;
    // the second isn't ours to write to. Keep it in the per-version runtime
    // dir, the same isolation the data dir gets. Windows has no socket file.
    let unix_paths = if cfg!(windows) {
        String::new()
    } else {
        let parent = posix(data_dir.parent().unwrap_or(data_dir));
        format!(
            "socket = \"{parent}/mysql.sock\"\n\
             pid-file = \"{parent}/mysql.pid\"\n"
        )
    };
    format!(
